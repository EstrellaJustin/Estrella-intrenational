...
网站打开后直接进入「全球出国项目分类系统」：一级分类 → 展开二级分类 → 进入项目列表页。
首页唯一任务：让用户找到全球出国项目（无营销模块 / 无统计 / 无热门推荐 / 无 AI 入口按钮）。
...
| 页面 | 说明 |
| --- | --- |
| `index.html` | **首页 = 全球出国项目分类系统**：标题 + 十大一级分类（点击展开二级分类，点击进入项目列表页） |
...
交互：点击一级分类 → 0.3s 平滑展开二级分类 → 点击二级分类进入项目列表页（如「高技能工作签证」→ 美国 H-1B / 德国 EU 蓝卡 / 日本高度人才签证 等 53 个项目）。
...
- 首页仅保留分类系统：无搜索/统计/热门国家/AI 入口按钮/营销 Banner
- 一级分类展开收起 0.3s 平滑；二级分类 → 项目列表页（H-1B / EU 蓝卡 / 高度人才等）
