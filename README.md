...
## AI 全球身份规划深度评估中心（10 阶段）

- 流程：基础身份信息 → 出国核心目的（多选+优先级排序）→ 职业与能力背景 → 教育背景 → 语言能力 → 经济情况 → 家庭规划 → 生活方式偏好（气候/天气接受度）→ 城市与社会偏好 → 价值观与未来规划
- 进度：Step 01/10 标识 + 蓝色进度条 + 10 段进度点，步骤切换 0.3s 平滑过渡
- 生成：数据分析动画 → 《全球身份规划分析报告》
- 报告五部分：① 用户画像 ② 国家匹配排名（前 5 名 + 匹配度 + 原因）③ 推荐项目（3 个，可打开详情）④ 不推荐项目（附原因）⑤ 个性化路线（未来 3 年三阶段规划）
- 匹配引擎：基于 430 项目 / 53 国数据的客户端智能评分（目的 / 职业 / 预算 / 家庭 / 学历 / 语言 / 气候 / 城市偏好）
- UI：深蓝私人银行风，深色透明输入框 + 白底深蓝字按钮，无白色表单
